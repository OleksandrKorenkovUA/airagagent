...
[Детальніше](https://milvus.io/docs/install_standalone-windows.md)
...
-  Якщо виникає помилка **ConnectionError: Failed to connect to Ollama** це означає що ollama спробувала встановити TCP-з’єднання з локальним портом 11434, але там не було процеса, що слухає цей порт, тому операційна система негайно відхилила запит. Найпростіше на Windows, де Milvus уже працює через Docker, розгорнути саму Ollama у контейнері й пробросити той самий порт. Також перед викликом застосунку додайте 
